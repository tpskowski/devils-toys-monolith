  ...
  "characterCreation": {
    ...
    "steps": [
      ...
      },
      {
        ...
        "optional": true,
        "tables": [
          { "firstOf": ["Male Names", "Female Names", "Ambiguous Names"], "choose": true, "column": "Result" },
...
- **The player picks which of the three first-name tables their name comes off.** `choose: true` on the `firstOf` is what says so. Without it the application picks one at random, which is right for a hireling rolled up whole and wrong here: whether a character's given name comes off Male, Female, or Ambiguous Names is a decision about that character, and a player left to the coin rolls again until it agrees with them. The die is still the server's — the choice only says which of the three tables it is thrown on, and it is checked against the three this step declares.
- **A background's talent goes in the Talents panel, not in a slot.** Every background table that rolls one writes it as `**Hot-Wiring:** You know how to cross the right wires` — a name, a colon, and what it does — which is exactly what the sheet's `abilities` field holds. Each dossier screen and the Starting Gear review name `entryInto: { "field": "abilities" }`, so the player can file such a result as a talent beside filing it as equipment or as description, and the application takes the name off the front of the line rather than making them retype it. Filing it as gear was the only option before, which cost a slot for something nobody carries.
